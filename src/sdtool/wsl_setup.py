...
def get_shrink_setup_button_label(report: WslAvailabilityReport, config: WslPiShrinkConfig | None = None) -> str:
    ...
    if report.code == "missing_pishrink":
        return f"Install / Repair PiShrink in {distro_name} (Step 3 of 3)"
...
def build_shrink_setup_confirmation_text(report: WslAvailabilityReport, config: WslPiShrinkConfig | None = None) -> str:
    ...
    if report.code == "missing_wsl":
        action = (
            ...
            f"and continue with Step 2 of 3 to install {distro_name}, followed by Step 3 of 3 to install PiShrink and required tools."
        )
        ...
        action = (
            ...
            "return to the app, click Re-check Shrink Readiness, and continue with Step 3 of 3."
        )
    elif report.code == "missing_pishrink":
        action = (
            f"This will start Step 3 of 3 and install or repair PiShrink inside the WSL distro {distro_name}.\n\n"
            f"It will also install required tools: {', '.join(_REQUIRED_APT_PACKAGES)}.\n\n"
        ...
        action = f"Shrink support already looks ready in {distro_name}. This will rerun the PiShrink install/repair step and required-tool setup inside that distro."
    else:
        action = "This will open the shrink setup helper and attempt to install or repair the missing parts of WSL/PiShrink."
    return f"Current status: {report.summary}\n\nDetails: {report.detail}\n\n{action}\n\nContinue?"
